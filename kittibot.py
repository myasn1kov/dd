...
def wake_up(update, context):
    ...
    context.bot.send_message(chat_id=chat.id,
                             text='Спасибо, что включили меня, {}!'.format(
                                 name),
...
def say_hi(update, context):
    ...
    context.bot.send_message(
        chat_id=chat.id, text='Саша получил ваше сообщение!')
...
def main():
    ...
    updater.dispatcher.add_handler(
        MessageHandler(Filters.text('Статус Саши'), status))
    updater.dispatcher.add_handler(MessageHandler(
        Filters.text('Обедал ли Саша?'), dinner))
    updater.dispatcher.add_handler(MessageHandler(
        Filters.text('Фото котика'), new_cat))
